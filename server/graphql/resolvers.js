...
const resolvers = {
  Query: {
    ...
    // get all events
    events: async () => {
      return await Event.find().sort({ date: -1 }).populate('host').populate('attendees').populate('comments').populate({ path: 'comments', populate: 'author' }).select('-__v');
    },
    ...
    goodDeeds: async () => {
      return await GoodDeed.find().sort({ date: -1 }).populate('host').populate('helper').populate('comments').populate({ path: 'comments', populate: 'author' }).populate({ path: 'comments.replies', populate: 'author' }).select('-__v')
    },
