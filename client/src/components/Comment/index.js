        ...
        <div className="divide-y-2 divide-gray-100">
          <div className="flex flex-wrap py-8 md:flex-nowrap">
            <div className="flex flex-col flex-shrink-0 mb-6 md:w-64 md:mb-0">
              ...
              {comment.createdAt && <span className="mt-1 text-sm text-gray-500">
                {comment.createdAt}
              </span>}
            </div>
          ...
          <div>
            {!viewReplies && comment.replies.length >= 1 && <button onClick={() => { setViewReplies(true) }}>View Replies</button>}
            {viewReplies && comment.replies.length >= 1 &&  <button onClick={() => { setViewReplies(false) }}>Hide Replies</button>}
          </div>
          <div>
